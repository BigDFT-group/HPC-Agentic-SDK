...
`irene-hpc` (adapted to the target scheduler and filesystem).
...
- **AI4S / GB200** (`Rikyu-Agent`) — a **GPU-first** cluster. Jobs are GPU jobs;
  ...
  a GPU.
- **IRENE BigWaterfall2** (`Irene-Agent`) — a **CPU-first** cluster. The bulk
...
`rikyu`→`irene` — it was **flipping the defaults and emphasis** so the plugin
...
```bash
uv tool run --quiet --from git+https://github.com/RIKEN-RCCS/Irene-Agent.git@main#subdirectory=server irene-hpc-mcp
uv tool run --quiet --from git+https://github.com/RIKEN-RCCS/Irene-Agent.git@main#subdirectory=server irene-docs-mcp
```
...
- `server/irene_mcp/data/irene_config.json` — replace with the new machine's static facts.
- `server/irene_mcp/data/docs_index/` — rebuild from the new machine's documentation.
...
- Point `DOCS_SOURCE` at the new machine's original markdown guide and set
  `DOCS_SITE_BASE`.
- Keep the env-var precedence chain: `IRENE_HOST`, `IRENE_ACCOUNT`,
  `IRENE_EMBED_API_KEY`, `IRENE_CONFIG`.
...
- Scripts are written under `~/.irene/jobs/<name>-<timestamp>.sh` via
...
**`server/irene_mcp/data/irene_guide.md`** — an *original*, plain-language
guide we wrote (see "Write the guide, don't copy it" below), so `ingest.py` just
chunks markdown by heading:

```bash
python -m irene_mcp.rag.ingest             # bundled guide + embeddings
python -m irene_mcp.rag.ingest --no-embed  # keyword-only index
```
...
`./plugins/irene`. Do not point Codex at `./`; the CLI may add the marketplace
...
| sbatch exits 0 but job never appears | script syntax error | check `~/.irene/jobs/` script manually |
