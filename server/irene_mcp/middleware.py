...
- Commands run under a login shell (Bridge/Slurm on Irene resolves its configuration
...
def get_frontend() -> Computer:
    """The (cached) Computer targeting the Irene front-end (login) node."""
    kwargs = {
        "template": "#!/bin/bash -l",
    ...
    passfile = config.ssh_passfile()
    if passfile:
        kwargs["passfile"] = passfile
    return Computer(**kwargs)


def run_command(cmd: str, raise_errors: bool = True) -> str:
    ...
    with contextlib.redirect_stdout(sys.stderr):
        try:
            ...
            if not config.CONFIG_PATH.exists():
                raise RuntimeError(
                    "Plugin not configured — run the 'irene-configuring' skill to "
