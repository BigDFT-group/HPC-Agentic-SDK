"""Build the documentation index from the IreneAgent guide.

The docs source is `irene_mcp/data/irene_guide.md` — an original, plain-language guide to
TGCC Irene written for users working through the agent. (It is *not* a
...
and writes irene_mcp/data/docs_index/chunks.json (+ embeddings.npy).

    python -m irene_mcp.rag.ingest                  # bundled guide + embeddings
    python -m irene_mcp.rag.ingest --source FILE    # use a specific markdown file
    python -m irene_mcp.rag.ingest --no-embed       # keyword-only index

Embeddings use the configured embedding endpoint (config.EMBED_BASE_URL / EMBED_MODEL)
and require an API key (IRENE_EMBED_API_KEY or embedding.api_key in the config
...
def build_index(source: Path, out_dir: Path, embed: bool) -> None:
    ...
    if not config.embed_api_key():
        ...
        print("No embedding API key configured — wrote a BM25-only index "
              "(set IRENE_EMBED_API_KEY and re-run to add vectors).")
        return

    import numpy as np

    from irene_mcp.rag.embed import get_client
    from irene_mcp.rag.store import chunk_text
